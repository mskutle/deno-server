import { fauna, dotenv } from "./deps.ts";

const DB_SECRET = Deno.env.get("DB_SECRET") ?? dotenv.config()?.DB_SECRET;
...
export async function update<T>(withIndex: string, id: string, obj: T) {
  const fql = Replace(Select("ref", Get(Match(withIndex, id))), { data: obj });
...
export async function get<T>(withIndex: string, id: string) {
  const fql = Get(Match(withIndex, id));
  const res = (await client.query(fql)) as Document<T>;
  return res?.data;
}

export async function getAll<T>(withIndex: string) {
  const fql = Map(Paginate(Match(withIndex)), (o) => Select("data", Get(o)));
  const res = (await client.query(fql)) as Document<T[]>;
  return res?.data;
}

export async function remove<T>(withIndex: string, id: string) {
  const fql = Delete(Select("ref", Get(Match(withIndex, id))));
  const res = (await client.query(fql)) as Document<T>;
  return res?.data;
}
